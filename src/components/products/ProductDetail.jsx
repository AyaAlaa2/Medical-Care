...
import {
  ...
  Paper,
  Typography,
  Accordion, // New MUI Component
  AccordionSummary, // New MUI Component
  AccordionDetails, // New MUI Component
  List, // New MUI Component for better list formatting
  ListItem, // New MUI Component
  ListItemIcon, // New MUI Component
  ListItemText, // New MUI Component
} from "@mui/material";
...
const ProductDetail = ({ product }) => {
  // نستخدم هنا حالة "expanded" لتحديد الأكورديون المفتوح
  const [expanded, setExpanded] = useState("ingredients");

  // معالجة البيانات كما كانت سابقاً
  const ingredients =
    ...
    <Box sx={{ mt: 4, px: { xs: 1, md: 0 } }}>
      ...
      <Paper
        ...
        sx={{
          ...
          border: "1px solid",
          borderColor: "divider",
          p: { xs: 1, md: 1 }, // تقليل الـ padding للـ Paper ليتسع للأكورديون
        }}
      >
        {/* DESCRIPTION & USE ACCORDION */}
        <Accordion
          expanded={expanded === "description"}
          onChange={handleChange("description")}
          disableGutters // لإزالة الهوامش الداخلية
        ...
        >
          <AccordionSummary
            ...
            sx={{
              backgroundColor: expanded === "description" ? "#f5f5f5" : "inherit",
          ...
          <AccordionDetails>
            {hasDescription ? (
              <Typography variant="body2" sx={{ whiteSpace: "pre-line" }}>
                {product.description || "Used to prevent blood clots in patients who have had a recent heart attack or stroke, or who have peripheral arterial disease."}
              </Typography>
            ) : (
        ...
        >
          <AccordionSummary
            expandIcon={<ExpandMoreIcon color="var(--main-color)" />}
            sx={{
              backgroundColor: expanded === "ingredients" ? "#f5f5f5" : "inherit",
              "&:hover": { backgroundColor: "#f0f0f0" },
            }}
          >
        ...
        >
          ...
          <AccordionDetails>
            ...
            {hasCare ? (
              <Typography variant="body2" sx={{ whiteSpace: "pre-line" }}>
                {product.careInstructions || "Consult a physician for dosage and use."}
              </Typography>
            ) : (
              <Typography variant="body2" color="text.secondary">
